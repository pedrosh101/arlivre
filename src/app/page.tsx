...
export default function Home() {
  return (
    <div
      className="flex flex-col sm:flex-row w-full sm:p-4 px-3 pt-4 sm:space-x-4 bg-cover bg-no-repeat bg-top"
      style={{
        backgroundImage:
    ...
    >
      ...
      <div className="order-2 sm:order-1 grid sm:grid-cols-3 grid-cols-1 sm:w-4/6 rounded-md md:bg-slate-100/20 bg-slate-100/40 mb-4 sm:mb-0 auto-rows-min">
        ...
        {[...projects].reverse().map((livro) => (
          <Card key={livro.id} project={livro} />
        ))}
      </div>
    </div>
  );
